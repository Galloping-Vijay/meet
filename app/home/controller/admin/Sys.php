...
class Sys extends AdminBase
{
    ...
    {
        if (!request()->isAjax()) {
            $this->error('提交方式不正确', url('index/admin.sys/admin_rule_list'));
        } else {
            $indexAuthRule = new AuthRule();
            $pid = $indexAuthRule->where(array('id' => input('pid')))->field('level')->find();
            $level = $pid['level'] + 1;
            $name = input('name');
            $name = $indexAuthRule->check_name($name, $level);
            if ($name) {
                $sldata = array(
                    'name' => $name,
                    'title' => input('title'),
                    'module_id' => Module::module_id('index'),
                ...
                $indexAuthRule->insert($sldata);
                Cache::clear();
                $this->success('权限添加成功', url('index/admin.sys/admin_rule_list'), 1);
            } else {
                $this->error('控制器或方法不存在,或提交格式不规范', url('index/admin.sys/admin_rule_list'));
            }
        }
    }
    ...
    public function admin_rule_order()
    {
        if (!request()->isAjax()) {
            ...
            Cache::clear();
            $this->success('排序更新成功', url('index/admin.sys/admin_rule_list'));
        }
    }
    ...
    public function admin_rule_runedit()
    {
        if (!request()->isAjax()) {
            ...
            //判断是否更改了pid
            if ($pid != $old_pid) {
                $level = $indexAuthRule->where('id', $pid)->value('level') + 1;
            ...
            if ($name) {
                ...
                $rst = $indexAuthRule->update($sldata);
                if ($rst !== false) {
                    if ($pid != $old_pid) {
                        //更新子孙级菜单的level
                        $auth_rule = $indexAuthRule->order('sort')->select();
                        $tree = new \Tree();
                        $tree->init($auth_rule, ['parentid' => 'pid']);
                        $ids = $tree->get_childs($auth_rule, $sldata['id'], true, false);
                        if ($ids) {
                            if ($level > $old_level) {
                                $indexAuthRule->where('id', 'in', $ids)->setInc('level', $level_diff);
                            } else {
                                $indexAuthRule->where('id', 'in', $ids)->setDec('level', $level_diff);
                            }
                        }
                    }
                    Cache::clear();
                    $this->success('权限修改成功', url('index/admin.sys/admin_rule_list'));
                } else {
                    $this->error('权限修改失败', url('index/admin.sys/admin_rule_list'));
                }
            } else {
                $this->error('控制器或方法不存在,或提交格式不规范', url('index/admin.sys/admin_rule_list'));
            }
    ...
    public function admin_rule_del()
    {
        ...
        if ($arrTree) {
            ...
            if ($rst !== false) {
                Cache::clear();
                $this->success('权限删除成功', url('index/admin.sys/admin_rule_list'));
            } else {
                $this->error('权限删除失败', url('index/admin.sys/admin_rule_list'));
            }
        } else {
            $this->error('权限删除失败', url('index/admin.sys/admin_rule_list'));
        }
